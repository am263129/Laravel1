<template>
  <div class="xjkax">
    <div class="col-12 videosongs-page">
      <!-- END Spinner -->
      <collection-modal
        ...
        <div class="btn-group">
          <div class="dropdown ml-1">
            <button
              ...
              aria-haspopup="true"
              aria-expanded="false"
            >{{$t(trendingActiveTranslate)}}</button>
            ...
            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownTrendingButton">
              <a
                ...
                @click="activeTrending = item.value; trendingActiveTranslate = item.translate"
              >{{$t(item.translate)}}</a>
            </div>
          </div>
          <div class="dropdown ml-1">
            <button
              ...
              aria-haspopup="true"
              aria-expanded="false"
            >{{$t(genreActiveTranslate)}}</button>

            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownGenreButton">
              <a
                class="dropdown-item"
                v-for="(item, index) in genereList"
                :key="index"
                :class="{'active': activeGenre == item.value}"
                @click="activeGenre = item.value;  genreActiveTranslate = item.translate"
              >{{$t(item.translate)}}</a>
            </div>
          </div>
        </div>
      ...
      <div class="col-12" v-if="data.videosongs !== null">
        <div class="row">
          <div
            class="col-6 col-md-4 col-lg-3 col-xl-1-5 col-xxl-1-5 p-2 p-md-4 mt-3"
            v-for="(item, index) in data.videosongs"
            :key="index"
          >
            <transition name="slide-down-fade">
              <div class="animation" v-show="showSlideUpAnimation">
                <div class="poster" @mouseover="ACTIVE_SLELECTED_VIDEOSONG(item.id)">
                  <router-link :to="{name: 'videosong-player', params: {id: item.id}}">                  
                          ...
                          <div class="mt-0">
                            ...
                            >
                              <img
                                ...
                                alt="favor"
                                width="100%"
                                @click.prevent="SHOW_COLLECTION_MODAL(item.id, '/storage/backdrops/600_' + item.backdrop, item.name, 'videosong', index)"
                                v-if="item.cloud == 'local'"
                              />
                              <img
                                src="/themes/default/img/infavor.svg"
                                alt="favor"
                                width="100%"
                                @click.prevent="SHOW_COLLECTION_MODAL(item.id, md_backdrop + item.backdrop, item.name, 'videosong', index)"
                                v-if="item.cloud == 'aws'"
                              />
                            </div>
                            ...
                            >
                              <img
                                ...
                                alt="favor"
                                width="100%"
                                @click.prevent="DELETE_FROM_COLLECTION(item.id, 'videosong', index)"
                              />
                            </div>
                          </div>
                          ...
                          <div class="mt-1">
                            <div class="poster__add-like-icon text-right" v-if="! item.is_like">
                              <img
                                ...
                                alt="dislike"
                                width="100%"
                                @click.prevent="ADD_NEW_LIKE(item.id, 'videosong', index, 'add')"
                              />
                            </div>
                            ...
                            <div class="poster__remove-like-icon text-right" v-if="item.is_like">
                              <img
                                ...
                                alt="like"
                                width="100%"
                                @click.prevent="ADD_NEW_LIKE(item.id, 'videosong', index, 'delete')"
                              />
                            </div>
                          </div>
...
export default {
  ...
  watch: {
    sort() {
      this.$store.dispatch("GET_VIDEOSONGS_SORT_BY_LIST", {
  ...
  mounted() {
    if (this.data.length == 0 || this.data === null) {
      if (this.$auth.isAuthenticated()) {
        this.$store.dispatch("GET_VIDEOSONGS_LIST");
      } else {
        this.$store.dispatch("GET_GHOST_VIDEOSONGS_LIST");
      }
    }    
    setTimeout(() => {
  ...
  methods: {
    ...
    HIDE_COLLECTION_MODAL_SAVE() {
      if (this.$auth.isAuthenticated()) {
        this.collection.id = null;
        this.data.videosongs[this.collection.index].is_favorite = true;
      } else {
        this.$router.push({
          name: "login"
        });
    ...
    DELETE_FROM_COLLECTION(id, type, index) {
      if (this.$auth.isAuthenticated()) {
        this.data.videosongs[index].is_favorite = false;
    ...
    ADD_NEW_LIKE(id, type, index, status) {
      if (this.$auth.isAuthenticated()) {
        if (status === "add") {
          ...
          this.data.videosongs[index].is_like = true;
          ...
          this.data.videosongs[index].is_like = false;
    ...
    ACTIVE_SLELECTED_VIDEOSONG(id) {
      this.active_videosong = id;
    }
  }
};
</script>
