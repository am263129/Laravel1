<template>
  <div>
    <div class="k1_manage_table">
      <h5 class="title p-2">Video Songs</h5>
      ...
      <div v-if="!spinner_loading">
                ...
                <div class="modal-body">
                  ...
                  <div class="col-12">
                    <input
                      ...
                      class="inputfile"
                    />
                    <label id="subtitleLabel" for="subtitle">
                      Add New Subtitles
                      <br />
                  ...
                  <div class="subtitle-progress mt-1 p-4" v-if="subtitle_video">
                    ...
                    <div class="progress">
                      <div
                        v-if="percentSubtitleUpload !== 100"
                        class="progress-bar"
                        role="progressbar"
                        :style="{width: percentSubtitleUpload + '%', height:'6px' }"
                        :aria-valuenow="percentSubtitleUpload"
                        aria-valuemin="0"
                        aria-valuemax="100"
                      ></div>
                      <div v-else>
                          ...
                          <td>
                            <div class="btn-group" role="group">
                              <button
                                ...
                                @click="DELETE_SUBTITLE(item.id,index)"
                              >Delete</button>
                              <button
        ...
        <div class="text-center" v-if="data.video_songs === null">
        ...
        <div v-else>
          <div
            class="mt-2"
            v-if="Object.keys(data_search).length  > 0  && data_search.video_songs !== null "
          >
            <div class="table table-hover">
              <thead>
                <th style="text-align: center;">
                  <input type="checkbox" id="select-all" @click="ADD_ALL_TO_MULITSELECT()" />
                </th>
                <th>Cover</th>
                <th>Name</th>
                <th>Year</th>
              ...
              <tbody>
                <tr v-for="(item, index) in data_search.video_songs" :key="index">
                  ...
                  <td>{{item.name}}</td>
                  <td>{{item.year}}</td>
                  <td v-if="item.m_cloud == 'local'">Local Server</td>
                  <td v-if="item.m_cloud == 'aws' ">AWS S3</td>
                  ...
                  <td>
                    <div class="btn-group" role="group">
                      <router-link
                        ...
                        :to="{name:'analysis-videosong', params: {id:item.id}}"
                        role="button"
                      >Analysis</router-link>
                      <button
                        class="btn btn-sm btn-warning btn-sm mr-2"
                        @click="ADD_TO_TOP(item.id,index)"
                        v-if="item.videosong_id !== item.id"
                        :id="item.id"
                      >Top</button>
                      <button
                        class="btn btn-sm btn-warning btn-sm mr-2"
                        @click="GET_SUBTITLE(item.id,index)"
                        data-toggle="modal"
                        data-target="#GetSubtitleModal"
                      >Get Subtitles</button>
                      <router-link
                        class="btn btn-sm btn-warning btn-sm mr-2"
                        :to="{name:'videosong_edit', params: {id:item.id}}"
                        role="button"
                      >Edit</router-link>
                    </div>
                  </td>
          ...
          <div
            ...
            v-if="data_search.video_songs === null || Object.keys(data_search).length === 0"
          >
            <div class="table table-hover">
              <thead>
                <th style="text-align: center;">
              ...
              <tbody>
                <tr v-for="(item, index) in data.video_songs.data" :key="index">
                  <td style="text-align: center;">
                    <input
                      type="checkbox"
                      :id="item.id"
          ...
          >
            <ul class="pagination">
              <li class="page-item" id="end">
                <a class="page-link" @click="PAGINATION('/api/admin/get/video_songs')">Begin</a>
              </li>
              <li
                class="page-item"
                id="prev"
                :class="{disabled: data.video_songs.prev_page_url === null}"
              >
                <a class="page-link" @click="PAGINATION(data.video_songs.prev_page_url)">Previous</a>
              </li>
              <li class="page-item">
                <a
                  class="page-link"
                >{{data.video_songs.current_page}}/{{data.video_songs.last_page}}</a>
              </li>
              <li
                class="page-item"
                id="next"
                :class="{disabled: data.video_songs.next_page_url === null}"
              >
                <a class="page-link" @click="PAGINATION(data.video_songs.next_page_url)">Next</a>
              </li>
              <li class="page-item" id="end">
                <a
                  class="page-link"
                  @click="PAGINATION('/api/admin/get/video_songs?page='+data.video_songs.last_page)"
                >End</a>
              </li>
            </ul>
          </nav>
...
export default {
  ...
  computed: mapState({
    data: state => {
      // console.log(state.videos.data.video_songs.data);
      return state.videos.data;
    },
    data_search: state => state.videos.data_search,
    button_loading: state => state.videos.button_loading,
    spinner_loading: state => state.videos.spinner_loading,
    // subtitles: { subtitles: [] }
    // subtitles: state => state.videos.subtitles.data
  ...
  created() {
    this.$store.commit("CLEAN_SEARCH_VIDEOSONGS");
    this.$store.dispatch("GET_VIDEOSONGS");
  },
  ...
  methods: {
    DELETE(id, key) {
      swal({
        ...
        if (willDelete) {
          this.$store.dispatch("DELETE_VIDEOSONG", this.multi_select);
        }
      });
    },

    PAGINATION(path_url) {
      this.$store.dispatch("GET_VIDEOSONGS_PAGINATION", path_url);
    },

    SEARCH() {
      if (this.query.length > 0) {
        this.$store.dispatch("GET_VIDEOSONG_SEARCH", this.query);
      } else {
        this.data_search.video_songs = null;
      }
    },

    ADD_TO_TOP(id, key) {
      this.$store.dispatch("ADD_VIDEOSONG_TO_TOP", {
        id,
        key
      });
    },
    ...
    GET_SUBTITLE(id, key) {
      ...
      this.subtitle_videosong_id = id;
      this.$store.dispatch("GET_VIDEOSONG_SUBTITLES", id);
    },
    ...
    UPLOAD_SUBTITLE(id) {
      ...
      formData.append("id", this.subtitle_videosong_id);
      ...
      axios
        .post("/api/admin/new/videosong/video_songsubtitle", formData, progress)
        .then(
          response => {
    ...
    AVAILABLE_IT(id, index) {
      axios
        .post("/api/admin/update/videosong/available", {
          ...
          response => {
            if (response.status === 200) {
              for (let i = 0; i < response.data.list.length; i++) {
                this.data.video_songs.data[response.data.list[i].key].show =
                  response.data.list[i].show;
              }
            }
          },
          error => {
            console.log("error in available videosong ");
          }
        );
    },
    ...
    ADD_ALL_TO_MULITSELECT() {
      if (this.multi_select.length > 0) {
        ...
        for (let i = 0; i < this.data.video_songs.data.length; i++) {
          this.multi_select.push({
            id: this.data.video_songs.data[i].id,
            key: i
          });
          document.getElementById(
            this.data.video_songs.data[i].id
          ).checked = true;
        }
      }
    }
